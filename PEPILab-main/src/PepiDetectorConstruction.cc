...
void PepiDetectorConstruction::DefineVolumes()
{
  ...
  fPixiRadSize2Y = fPixelSize2Y * fnPixelsY;
  ...
  // ========================================
  //                DETECTOR MASK M2 and SUBSTRATE esto también está modificado OJO!!!
  // ========================================
...
/*  
 ...
 fPixelSolid2 = new G4Box("Pixel2",                          //its name
                          fPixelSize2X/2,                    //its size
                          fPixelSize2Y/2,
                          fPixelSize2Z/2);
                     
  fPixelLogical2 = new G4LogicalVolume(fPixelSolid2,          //its solid
 ...
 // Build the Speckel's Envelope 
  fSandSolid =  new G4Box("SandSV",                    //its name                 
 ...
 G4double radius = 200*um;
//  G4Material* silicon = nist->FindOrBuildMaterial("G4_Si");
  //G4Box* sandpaper = new G4Box("sand_paper",0.005*m, 0.005*m, 0.005*m);
   
  fSphereSolid = new G4Sphere("speckle", 0, radius, 0, 360*deg, 0, 180*deg);
  fSphereLogical = new G4LogicalVolume(fSphereSolid, fSubMaterial, "speckLV"); 
  
 G4int copy_nn = 0;
 
 for(G4int i = 0; i < fnPixelsY; i ++)
 {
 	for(G4int j = 0; j < fnPixelsX; j ++)
 	{
 	G4double x1 = + j*fPixelSize2X - fPixiRadSize2X/2;// + fPixelSizeX/2;
       G4double y1 = - i*fPixelSize2Y + fPixiRadSize2Y/2;// - fPixelSizeY/2;
      
      G4ThreeVector position1 = G4ThreeVector(x1, y1, 0);
      G4String  names = "Speckle_" + G4UIcommand::ConvertToString(copy_nn);
      
      		fPixelPhysical2 = new G4PVPlacement(0, position1, fPixelLogical2, names, fWorldLogical, false, copy_nn, fCheckOverlaps); 
      
 //		G4VPhysicalVolume* phys_sand = new G4PVPlacement(0, G4ThreeVector(-0.005*m +(i+0.005)*m/100, -0.005*m+(j+0.005)*m/100, 0.005*m), logic_sand, "phys_sand", fWorldLogical, false, i+j*fnPixelsX, true); 
	copy_nn++;   
 	} 
 }                            
 
 // - Place the Sandpaper Envelope in the World
  G4ThreeVector positionSand = G4ThreeVector(0, 30*um, fSourcePosZ+fSrcObjDistance -5*cm);
  fPixiRadPhysical = new G4PVPlacement(0,                                                  //its rotation
  ...
  G4Region* aRegion2 = new G4Region("LogicSand");
...
// Cambiamos los objetos para ver solo uno
...
//
  ...
  */                                    
  ...
  // ========================================
  //       SAMPLE MASK M1 and substrate          esto es lo que modifiqué OJO!!!!!!!
  // ========================================
...
*/  
  ...
  G4VisAttributes* sphereVisAtt = new G4VisAttributes(G4Colour(0.6,0.8,1.0));
//  objectVisAtt2->SetForceWireframe(true);
  sphereVisAtt->SetForceSolid(true);
  // objectVisAtt->SetForceAuxEdgeVisible(true);     
  fSphere2Logical->SetVisAttributes(sphereVisAtt);
...
//  objectVisAtt3->SetForceWireframe(true);
 ...
 /* 
  G4VisAttributes* pixel2VisAtt = new G4VisAttributes(G4Colour(0.0,0,1.0));
  pixel2VisAtt->SetForceWireframe(true);
  //pixelVisAtt->SetForceSolid(true);
  pixel2VisAtt->SetVisibility(false);
  // pixelVisAtt->SetForceAuxEdgeVisible(true);
  fPixelLogical2->SetVisAttributes(pixel2VisAtt);
*/
  ...
  /*
   G4VisAttributes* speckleVisAtt = new G4VisAttributes(G4Colour(0.0,0,1.0));
  speckleVisAtt->SetForceWireframe(true);
  //pixelVisAtt->SetForceSolid(true);
  speckleVisAtt->SetVisibility(false);
  // pixelVisAtt->SetForceAuxEdgeVisible(true);
  fSphereLogical->SetVisAttributes(speckleVisAtt);
*/
/*
  G4VisAttributes* sandVisAtt = new G4VisAttributes(G4Colour(1.0,0,0.0));
...
// aquí se modifica para dehar un solo objeto 
 // object 2
 /*
  ...
  */
  ...
  {
 // Modifica la máscara y los cortes, así se deja ejecutar en inline 
...
// ESTO ES MODIFICADO PARA EVITAR ERRORES EN LA MASCARA M1
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// MODIFICANDO PARA QUITAR PERMANENTEMENTE LAS MASCARAS QUE NO NECESITO
